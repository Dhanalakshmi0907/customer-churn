...
# Step 1: Read and prepare the dataset
filename = 'customer_churn.csv'

ages = []  # Replace 'MonthlyCharges' with 'Age' if renamed earlier
tenures = []
total_charges = []
...
with open(filename, 'r') as file:
    ...
    for row in reader:
        try:
            age = float(row['MonthlyCharges'])  # Replace with 'Age' if renamed
            tenure = float(row['Tenure'])
...
normalized_tenures = normalize_data(tenures)
normalized_total_charges = normalize_data(total_charges)

# Combine normalized features into a single dataset
dataset = list(zip(normalized_ages, normalized_tenures, normalized_total_charges))
...
def k_means(dataset, k=3, max_iters=100):
    ...
    for _ in range(max_iters):
        # Step 3.1: Assign data points to the nearest centroid
        ...
        for i in range(k):
            if clusters[i]:
                ...
                new_centroids.append(centroids[i])  # Keep the old centroid if no points were assigned
        centroids = new_centroids

    return clusters, centroids

# Apply K-Means to the dataset
clusters, centroids = k_means(dataset, k=3)

# Step 4: Visualize the clusters (2D plot)
# Extract data for plotting
clustered_data = {i: {'x': [], 'y': []} for i in range(3)}
for i, cluster in clusters.items():
    for point in cluster:
        clustered_data[i]['x'].append(point[0])  # Normalized Age
        clustered_data[i]['y'].append(point[1])  # Normalized Tenure

# Plot the clusters
for i in range(3):
    plt.scatter(clustered_data[i]['x'], clustered_data[i]['y'], label=f'Cluster {i+1}')

plt.scatter([centroid[0] for centroid in centroids], [centroid[1] for centroid in centroids], color='black', marker='X', label='Centroids')
plt.title("Customer Segmentation (Clustering with K-Means)")
plt.xlabel("Normalized Age")
plt.ylabel("Normalized Tenure")
plt.legend()
plt.show()
...
# Read data from CSV and handle missing or invalid data
with open(filename, 'r') as file:
    reader = csv.DictReader(file)
    for row in reader:
        try:
...
exited = []  # Customer churn status (1 if churned, 0 if not)

with open(filename, 'r') as file:
    reader = csv.DictReader(file)
    for row in reader:
        try:
            tenure.append(int(row['Tenure']))  # Tenure of customer (in months)
            exited.append(int(row['Exited']))  # Exited (1 if churned, 0 if not)
        except (ValueError, KeyError):
            continue  # Skip rows with missing or invalid data

# Step 2: Time-based analysis
# Group the churn data based on tenure (e.g., bucket the tenure in ranges of months)
# For example: 0-12 months, 13-24 months, 25-36 months, etc.
tenure_buckets = defaultdict(list)

# Create tenure buckets (e.g., 0-12, 13-24, 25-36 months)
for t, e in zip(tenure, exited):
    bucket = (t // 12) * 12  # Grouping customers by year of tenure (0-12, 12-24, etc.)
    tenure_buckets[bucket].append(e)

# Calculate churn rate for each bucket (time period)
churn_rate_by_tenure = {}

for bucket, churn_data in tenure_buckets.items():
    churn_rate = sum(churn_data) / len(churn_data)  # Churn rate = number of churned / total customers in bucket
    churn_rate_by_tenure[bucket] = churn_rate

# Step 3: Visualize churn rate by tenure bucket
# Sorting tenure buckets for plotting
sorted_buckets = sorted(churn_rate_by_tenure.keys())

# Plotting churn rate by tenure (in months)
x = [bucket for bucket in sorted_buckets]
y = [churn_rate_by_tenure[bucket] for bucket in sorted_buckets]

plt.figure(figsize=(10, 6))
plt.plot(x, y, marker='o', linestyle='-', color='b', label='Churn Rate')
plt.title("Churn Rate by Tenure")
plt.xlabel("Tenure (Months)")
plt.ylabel("Churn Rate")
plt.xticks(np.arange(min(x), max(x)+1, 12))  # Show months in 12-month intervals
plt.grid(True)
plt.legend()
plt.show()

# Step 4: Time to churn analysis (average time before churn)
# Calculate the average time before churn (for customers who have churned)
churned_tenure = [t for t, e in zip(tenure, exited) if e == 1]
average_churn_time = np.mean(churned_tenure)

print(f"Average time to churn (in months): {average_churn_time:.2f}")
